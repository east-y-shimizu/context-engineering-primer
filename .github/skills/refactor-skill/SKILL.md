...
リファクタリングを実装する前に、コードを整理（Tidy）することを推奨します：

1. **構造の変更を先に**: 動作を変えずに、コードの構造を改善
2. **動作の変更を後で**: 新機能の追加やバグ修正は、構造が整理された後に実施
...
リファクタリング後のコミット・PR作成については、プロジェクトのGit/GitHub運用スキル（`.github/skills/git-skill`）が存在する場合はそれを参照してください。

### コミットメッセージ

- タイプ: `refactor` を使用
- 例: `refactor: Extract common parser logic to base class`

### PR作成

- ラベル: `refactor` を使用
- リファクタリングのPRでは、動作が変わらないことを確認するテスト結果を含めてください
...
リファクタリングのコミットは以下のように分離することを推奨します：

1. **構造の変更（Tidy First）**: 動作を変えない、純粋なリファクタリング
   - 変数名の変更、関数の抽出、コードの移動など
   - コミットメッセージ: `refactor: Extract validation logic`

2. **動作の変更**: 新機能の追加やバグ修正
   - コミットメッセージ: `feat: Add CSV validation` または `fix: Handle empty values`

この分離により、レビューが容易になり、問題が発生した場合の特定が簡単になります。
...
プロジェクトに以下のスキルやドキュメントが存在する場合は参照してください：

- **Git/GitHub運用スキル**（`.github/skills/git-skill`）: リファクタリングのコミットメッセージ形式（`refactor:`）、PR作成、ブランチ命名規則
- **TDD開発スキル**（`.github/skills/tdd-skill`）: テスト駆動リファクタリング、Red-Green-Refactor サイクルのRefactorフェーズ
- **リポジトリガイドライン**（例: `README.md`）: プロジェクトの運用ルール
- **コーディング規約**（例: `docs/CODING_STANDARDS.md`）: コーディングスタイル、命名規則
